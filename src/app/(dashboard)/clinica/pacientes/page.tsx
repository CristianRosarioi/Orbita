    ...
    <div className="p-6">
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-dashed border-slate-200 py-16 text-center">
          ...
          {!busqueda && (
            <Link href="/clinica/pacientes/nuevo" className="mt-3 inline-block text-sm text-indigo-600 hover:underline">
            ...
            <tbody className="divide-y divide-slate-100">
              {pacientes.map((p) => (
                <tr key={p.id} className="hover:bg-slate-50">
                  <td className="px-4 py-3 font-mono text-xs text-slate-500">#{p.numeroExpediente}</td>
                  ...
                  <td className="px-4 py-3">
                    <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${ESTADO_COLOR[p.estado] ?? 'bg-slate-100 text-slate-600'}`}>
