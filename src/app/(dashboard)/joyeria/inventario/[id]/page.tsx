...
const MATERIAL_LABEL: Record<string, string> = {
  ...
  PLATA_925: 'Plata 925', PLATINO: 'Platino', OTRO: 'Otro',
};
...
export default function DetallePiezaPage() {
  ...
  useEffect(() => {
    fetch(`/api/joyeria/inventario/${id}`)
      .then((r) => r.json())
      .then((d) => { if (d.success) setPieza(d.data); })
    ...
    <div className="p-6 max-w-2xl">
      ...
      <div className="mb-6 flex items-start justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">{pieza.tipo} · {MATERIAL_LABEL[pieza.material]}</p>
          </div>
        </div>
      ...
      <div className="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
        ...
        {pieza.precioCompra != null && (
          <div className="rounded-xl border border-slate-200 bg-white p-4 text-center">
            <p className="text-lg font-bold text-slate-900">RD$ {Number(pieza.precioCompra).toLocaleString('es-DO')}</p>
        ...
        <div className="rounded-xl border border-indigo-100 bg-indigo-50 p-4 text-center">
          <p className="text-lg font-bold text-indigo-900">RD$ {Number(pieza.precioVenta).toLocaleString('es-DO')}</p>
          ...
          <div className="space-y-2">
            {pieza.reparaciones.map((r) => (
              <div key={r.id} className="flex items-center justify-between rounded-lg border border-slate-200 bg-white px-4 py-3">
                ...
                <span className="text-xs text-slate-400">{new Date(r.createdAt).toLocaleDateString('es-DO')}</span>
              </div>
            ))}
          </div>
        </div>
