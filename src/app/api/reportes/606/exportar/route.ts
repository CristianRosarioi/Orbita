...
import { z } from 'zod';
import { facturaA606, generarTXT606, generarExcel606 } from '@/lib/exportar-reportes';
...
export async function GET(req: Request) {
  try {
    ...
    const nombreArchivo = `606_${anio}${String(mes).padStart(2, '0')}`;

    if (formato === 'xlsx') {
      const buffer = generarExcel606(registros);
    ...
    const txt = generarTXT606(registros);
    ...
    return handleApiError(error, 'GET /api/reportes/606/exportar');
  }
}
